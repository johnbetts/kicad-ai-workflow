...
def _add_component_specific_items(
    requirements: ProjectRequirements, items: list[ReviewItem],
) -> None:
    """Add antenna, relay, regulator, and zone-fill review items."""
    has_wifi, wifi_refs = _has_wifi_component(requirements)
    if has_wifi:
        items.append(ReviewItem(
            category="antenna", severity="required",
    ...
    if relay_refs:
        items.append(ReviewItem(
            category="relay", severity="required",
            title="Relay board slots",
            description="Add board slots between relay contacts and logic circuits",
            affected_refs=relay_refs,
        ))
    ...
    if regulator_refs:
        items.append(ReviewItem(
            category="thermal", severity="recommended",
            title="Regulator thermal vias",
            description="Add thermal vias under regulator thermal pad",
            affected_refs=regulator_refs,
        ))

    items.append(ReviewItem(
        category="mechanical", severity="required",
        title="Zone fill",
        description="Run zone fill (Edit \u2192 Fill All Zones / press B) before final DRC",
        affected_refs=(),
    ))


def _add_board_context_items(
    requirements: ProjectRequirements, items: list[ReviewItem],
) -> None:
    """Add review items from board context (system integration, ground topology)."""
    if requirements.board_context is None:
        return
    ctx = requirements.board_context
    if ctx.target_system:
        items.append(ReviewItem(
            category="context", severity="required",
    ...
    if ctx.shared_grounds:
        items.append(ReviewItem(
            category="context", severity="recommended",
    ...
    for note in ctx.notes:
        items.append(ReviewItem(
            category="context", severity="optional",
            title="Design note",
            description=note,
            affected_refs=(),
        ))
...
def generate_design_review(
    ...
    _add_component_specific_items(requirements, items)
    items.extend(_subcircuit_design_notes(requirements))
    _add_board_context_items(requirements, items)
