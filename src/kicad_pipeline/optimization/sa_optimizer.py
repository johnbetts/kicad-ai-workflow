...
def _perturbation_resolve_collision(
    ...
    # Clamp to board
    new_x = max(
        board_min_x + BOARD_EDGE_MARGIN_MM,
        min(board_min_x + board_w - BOARD_EDGE_MARGIN_MM, new_x),
    )
    new_y = max(
        board_min_y + BOARD_EDGE_MARGIN_MM,
        min(board_min_y + board_h - BOARD_EDGE_MARGIN_MM, new_y),
    )
...
def _sa_choose_perturbation(
    ...
    """Select and apply one SA perturbation step. Returns updated positions dict."""
    # Perturbation probability thresholds (cumulative):
    # 30% resolve collision, 25% pull connected, 20% nudge, 15% swap, 10% rotate
    roll = rng.random()
    if roll < 0.30:
        return _perturbation_resolve_collision(
            pos_dict, movable_refs, fp_sizes, rng, board_w, board_h, min_x, min_y,
        )
    if roll < 0.55:
        return _perturbation_pull_connected(
            pos_dict, movable_refs, adjacency, rng,
            temperature, board_w, board_h, min_x, min_y,
        )
    if roll < 0.75:
        return _perturbation_nudge(
            pos_dict, movable_refs, temperature, rng, board_w, board_h, min_x, min_y,
        )
    if roll < 0.90:
        return _perturbation_swap(pos_dict, movable_refs, rng)
    return _perturbation_rotate(pos_dict, movable_refs, rng)
...
def _sa_run_loop(
...
) -> tuple[PCBDesign, dict[str, tuple[float, float, float]]]:
    """Run the SA main loop. Returns (best_pcb, best_pos_dict)."""
    from kicad_pipeline.optimization.scoring import compute_fast_placement_score

    current_positions = _dict_to_positions(pos_dict)
    current_score = compute_fast_placement_score(
        _apply_positions(initial_pcb, current_positions), requirements,
    )
    ...
    for iteration in range(1, config.max_iterations + 1):
        new_pos_dict = _sa_choose_perturbation(
            pos_dict, movable_refs, fp_sizes, adjacency, rng,
            temperature, board_w, board_h, min_x, min_y,
        )
        ...
        if delta > 0 or rng.random() < math.exp(delta / sa_temp):
            ...
            history.append(PlacementCandidate(
    ...
    return best_pcb, _positions_to_dict(best_positions)
...
def optimize_placement_sa(
    ...
    Iteratively perturbs component positions (nudge/swap/rotate/collision-resolve/
    pull-connected), accepting improvements and probabilistic downgrades via the
    Boltzmann criterion. Returns the best-scoring PCB and the full accepted history.
    """
    ...
    best_pcb, _ = _sa_run_loop(
        config, initial_pcb, requirements, rng,
        _positions_to_dict(current_positions),
        movable_refs, fp_sizes, adjacency,
