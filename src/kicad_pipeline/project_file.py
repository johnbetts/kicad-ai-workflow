"""Generate KiCad 10 project files (.kicad_pro).
...
# ---------------------------------------------------------------------------
# Static KiCad project configuration fragments
# ---------------------------------------------------------------------------
...
_BOM_FIELDS: list[dict[str, object]] = [
...
def _build_design_settings() -> dict[str, Any]:
    """Build the board.design_settings section."""
    return {
        "defaults": {
        ...
        "rule_severities": dict(_RULE_SEVERITIES),
        "rules": dict(_DESIGN_RULES),
...
def _build_schematic_settings() -> dict[str, Any]:
    """Build the schematic section."""
    return {
        ...
        "bom_settings": {
            "exclude_dnp": False,
            "fields_ordered": [dict(f) for f in _BOM_FIELDS],
            "filter_string": "",
            "group_symbols": True,
...
def _build_default_netclass() -> dict[str, Any]:
    """Build the Default netclass entry."""
    return {
        "bus_width": 12,
        "clearance": 0.2,
        "diff_pair_gap": 0.25,
...
def build_project_file(
    ...
    data: dict[str, Any] = {
        ...
        "net_settings": {
            "classes": [_build_default_netclass()],
            "meta": {"version": 4},
            "net_colors": None,
        ...
        "schematic": _build_schematic_settings(),
        "sheets": sheets,
        "text_variables": {},
    }
